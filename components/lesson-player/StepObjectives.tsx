import type { ObjectivesStep } from "@/lib/types";
...
export function StepObjectives({ step, onContinue }: StepObjectivesProps) {
  return (
        ...
        <Card className="flex flex-col gap-3">
          {step.items.map((item, index) => (
            <div key={index} className="flex items-start gap-3">
              <span className="mt-0.5 flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-parlem-green-50 text-sm">
                🎯
              </span>
